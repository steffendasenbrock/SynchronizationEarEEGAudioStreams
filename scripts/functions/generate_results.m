% Script to create results struct (containing time, latency, jitter ...) 
% which is stored in the derivatives folder. 
...
% Clear everything
clc, clear, %close all
% Add search paths for data
addpath(genpath('./../../../functions/'))
% Search for all data which has 900s at the end
DIR = './../rawdata/';
experiments = {'stimulus_presentation_timing','eeg_isolated_timing','combined_stimulus_eeg_timing'};
search_strings = {'900s','3h'};
measurement_ranges = {[1:900], [1:10700]};

%% Loop through all experiments

for exp=1:3 % loop through all experiments
    labstream = {};
    for j=1:numel(search_strings)
        rawdata_list = dir([DIR experiments{exp} '/*' search_strings{j} '*.txt']);        
            for i=1:numel(rawdata_list)
                ...
                if exp==3
                   labstream{j,i} = import_labstreamer_data([rawdata_list(i).folder '/' rawdata_list(i).name],'External',true);
                end
            end        
    end
